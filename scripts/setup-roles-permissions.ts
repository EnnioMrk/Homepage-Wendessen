...
async function setupRolesAndPermissions() {
    ...
    try {
        ...
        `;
        console.log('✓ Added role_id and custom_permissions columns to admin_users');
        ...
        const existingPerms = await sql`SELECT COUNT(*) as count FROM permissions;`;
        ...
        if (superAdminRole.length > 0) {
            ...
            if (updatedUsers.length > 0) {
                console.log(`✓ Assigned Super Admin role to ${updatedUsers.length} existing user(s)`);
            }
        }
