...
async function testPortraitCleanup() {
    ...
    console.log(
        `📋 Configuration: MAX_REJECTED_PORTRAITS = ${PORTRAIT_CONFIG.MAX_REJECTED_PORTRAITS}`
    );
    ...
    try {
        ...
        if (cleanedUp === 0) {
            console.log(
                '✅ No cleanup needed - rejected portrait count is within limits'
            );
        } else {
            console.log(
                `🧹 Successfully cleaned up ${cleanedUp} old rejected portraits`
            );
        }
        ...
        if (wouldCleanup === 0) {
            ...
            console.log(
                `📊 With limit of 5, would clean up ${wouldCleanup} portraits`
            );
        }
