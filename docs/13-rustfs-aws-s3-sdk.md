...
RustFS is an S3 protocol-compatible object storage system, accessible through AWS's official JavaScript SDK (v3). This guide shows how to connect to RustFS and perform common object storage operations from Node.js.

## II. Preparation

### 2.1 SDK Installation

Install the required AWS SDK v3 modules with NPM (or Bun/Yarn):
...
const s3 = new S3Client({
    ...
    region: 'us-east-1', // arbitrary for many self-hosted S3 implementations
    credentials: {
    ...
    document
        ...
        .addEventListener('change', async (e) => {
            ...
            const url = await fetch(
                '/api/presigned-put-url?key=' + file.name
            ).then((r) => r.text());
