...
export function parseWeatherReport(reportText: string): WeatherData {
    ...
    return {
        ...
        wind: {
            ...
            direction: windDirIndex >= 0 ? (rawLines[windDirIndex + 1] ? rawLines[windDirIndex + 1].slice(25).trim().split(/\s+/)[0] : 'N') : 'N',
            degrees: windDirIndex >= 0 ? (getValue(windDirIndex + 1).numbers.find(n => n >= 0) || 0) : 0,
            directionDominant: windDirIndex >= 0 ? (rawLines[windDirIndex + 2] ? rawLines[windDirIndex + 2].slice(25).trim().split(/\s+/).pop() || 'N' : 'N') : 'N',
