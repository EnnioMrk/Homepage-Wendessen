...
const dorflebenItems = [
  {
    ...
    items: [
      ...
      { title: "Der Ortsbürgermeister berichtet", href: "/dorfleben/ortsrat/bericht" },
    ],
  },
  {
    title: "Vereinsleben",
    items: [
      ...
      { title: "Freiwillige Feuerwehr", href: "/dorfleben/vereine/feuerwehr" },
...
const wohnenBauenItems = [
  { title: "Neubaugebiet Leipziger Allee", href: "/wohnen-bauen/neubaugebiet" },
...
export default function MobileNavbar() {
  ...
  return (
    <div className="lg:hidden">
      ...
      >
        {open ? <X size={28} className="text-foreground" /> : <Menu size={28} className="text-foreground" />}
      </button>
      {open && (
      ...
      >
        ...
        <ul className="flex flex-col gap-2 p-4">
          <li>
            <Link href="/" className="block py-2 px-2 rounded text-foreground hover:bg-primary/20" onClick={() => setOpen(false)}>
              HOME
            </Link>
          </li>
          <li>
            <span className="block py-2 px-2 font-semibold text-foreground">DORFLEBEN</span>
            <ul className="pl-4">
              {dorflebenItems.map((item) => (
                <React.Fragment key={item.title}>
                  {item.href ? (
                    <li>
                      <Link href={item.href} className="block py-1 px-2 rounded text-foreground hover:bg-primary/20" onClick={() => setOpen(false)}>
                        {item.title}
                      </Link>
                    </li>
                  ) : (
                    <li className="font-medium py-1 text-foreground">{item.title}</li>
                  )}
                  {item.items && (
                    <ul className="pl-4">
                      {item.items.map((subItem) => (
                        <li key={subItem.title}>
                          <Link href={subItem.href} className="block py-1 px-2 rounded text-foreground hover:bg-primary/20" onClick={() => setOpen(false)}>
                            {subItem.title}
                          </Link>
                        </li>
                      ))}
          ...
          <li>
            <span className="block py-2 px-2 font-semibold text-foreground">WOHNEN & BAUEN</span>
            <ul className="pl-4">
          ...
          <li>
            <Link href="/was-steht-an" className="block py-2 px-2 rounded text-foreground hover:bg-primary/20" onClick={() => setOpen(false)}>
              WAS STEHT AN?
            </Link>
          </li>
          <li>
            <Link href="/geschichte" className="block py-2 px-2 rounded text-foreground hover:bg-primary/20" onClick={() => setOpen(false)}>
              GESCHICHTE
            </Link>
          </li>
          <li>
            <span className="block py-2 px-2 font-semibold text-foreground">KONTAKT</span>
            <ul className="pl-4">
