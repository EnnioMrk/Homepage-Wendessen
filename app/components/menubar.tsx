...
>(({ className, ...props }, ref) => (
  <MenubarPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex cursor-default select-none items-center rounded-sm px-3 py-2 text-sm font-medium text-foreground outline-none transition-colors hover:bg-primary/10 focus:bg-primary focus:text-primary-foreground data-[state=open]:bg-primary data-[state=open]:text-primary-foreground",
      className
    )}
    {...props}
  />
...
>(({ className, inset, ...props }, ref) => (
  <MenubarPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none transition-colors focus:bg-primary focus:text-primary-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50 hover:bg-primary/10",
      inset && "pl-8",
      className
    )}
    {...props}
...
>(({ className, inset, children, ...props }, ref) => (
  <MenubarPrimitive.SubTrigger
    ref={ref}
    className={cn(
      "flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none transition-colors focus:bg-primary focus:text-primary-foreground data-[state=open]:bg-primary data-[state=open]:text-primary-foreground hover:bg-primary/10",
      inset && "pl-8",
      className
    )}
    {...props}
...
export function MenubarDemo() {
  return (
    <Menubar className="rounded-none border-b border-none px-2 lg:px-4">
      ...
      <MenubarPrimitive.Menu>
        <MenubarTrigger className="font-semibold">DORFLEBEN</MenubarTrigger>
        <MenubarContent>
                ...
                <>
                  {item.items.map((subItem) => (
                    <Link key={subItem.title} href={subItem.href} passHref>
                      <MenubarItem className="pl-4">{subItem.title}</MenubarItem>
                    </Link>
                  ))}
                  <MenubarSeparator />
                </>
      ...
      <MenubarPrimitive.Menu>
        <MenubarTrigger className="font-semibold">WOHNEN & BAUEN</MenubarTrigger>
        <MenubarContent>
      ...
      <MenubarPrimitive.Menu>
        <Link href="/was-steht-an" passHref>
          <MenubarTrigger className="font-semibold">WAS STEHT AN?</MenubarTrigger>
        </Link>
      </MenubarPrimitive.Menu>

      <MenubarPrimitive.Menu>
        <Link href="/geschichte" passHref>
          <MenubarTrigger className="font-semibold">GESCHICHTE</MenubarTrigger>
        </Link>
      </MenubarPrimitive.Menu>

      <MenubarPrimitive.Menu>
        <MenubarTrigger className="font-semibold">KONTAKT</MenubarTrigger>
        <MenubarContent>
