...
export default function FeatureCard({
    ...
    const effectiveButtonColor = BUTTON_COLOR_CLASSES[buttonColor]
        ? buttonColor
        : "green";
    const effectiveHighlightColor =
        highlightColor && TEXT_HIGHLIGHT_CLASSES[highlightColor]
            ? highlightColor
            : effectiveButtonColor;
    const effectiveBackgroundColor =
        backgroundColor && BG_GRADIENT_CLASSES[backgroundColor]
            ? backgroundColor
            : effectiveButtonColor;
    const hasButton =
        Boolean(buttonText?.trim()) && Boolean(buttonHref?.trim());
    ...
    if (isTextOnly) {
        return (
            ...
            >
                <div
                    className={`flex-1 flex flex-col justify-end ${compact ? "p-6" : "p-6 md:p-8 lg:p-10"}`}
                >
                    <h3
                        className={`font-bold text-gray-900 ${isHero ? "text-left" : "text-center"} ${compact ? "text-xl mb-3" : "text-xl md:text-2xl lg:text-3xl mb-3 md:mb-4"}`}
                    >
                        {title}
                    </h3>
                    {subtitle && (
                        <p
                            className={`${subtitleClassName || TEXT_HIGHLIGHT_CLASSES[effectiveHighlightColor]} font-semibold ${isHero ? "text-left" : "text-center"} ${compact ? "text-sm mb-3" : "text-sm md:text-base lg:text-lg mb-3 md:mb-4"}`}
                        >
                            {subtitle}
                        </p>
                    )}
                    <div
                        className={`text-gray-700 leading-relaxed ${isHero ? "text-left" : "text-center"} ${compact ? "text-sm mb-6" : "text-sm md:text-base mb-6 md:mb-8"}`}
                    >
                        {description.split("\n").map((line, i) => (
                    ...
                    {hasButton && (
                        <div className={isHero ? "" : "text-center"}>
                            ...
                            >
                                ...
                                <ArrowRight
                                    className={`transform group-hover/btn:translate-x-1 transition-transform duration-300 ${compact ? "w-4 h-4" : "w-4 h-4 md:w-5 md:h-5"}`}
                                />
                            </a>
                        </div>
                    )}
    ...
    return (
        ...
        >
            <div className="relative w-full flex-1 overflow-hidden">
                ...
                <div
                    className={`absolute inset-0 flex flex-col justify-end text-white ${compact ? "p-6" : "p-6 md:p-8 lg:p-12"}`}
                >
                    <div
                        className={`${isHero && !compact ? "max-w-md lg:max-w-3xl" : ""}`}
                    >
                        <h3
                                    ...
                                    isHero
                                        ...
                                        : (subtitleClassName ||
                                          ...
                                          (compact
                                              ...
                                              : " text-lg md:text-xl lg:text-2xl mb-3 md:mb-4 text-center")
                                }`}
                            >
                                {subtitle}
                            </p>
                        ...
                        <div
                            className={`leading-relaxed drop-shadow-md opacity-90 ${
                                isHero
                                    ? compact
                                        ? "text-sm mb-4"
                                        : "text-sm md:text-base lg:text-xl mb-4 lg:mb-6"
